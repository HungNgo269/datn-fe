...
import { getPlanPromotions, deletePromotion } from "@/app/feature/promotions/api/promotions.api";
import { AdminPromotionList } from "@/app/feature/promotions/components/adminPromotionList";

function PlanPromotionsPageContent() {
  ...
  const { data: promotions, isLoading, isError, isFetching } = useQuery({
    queryKey: ["promotions", "subscription"],
    queryFn: getPlanPromotions,
  });
  ...
  if (isError)
    return (
      <div className="p-10 text-rose-600 text-center">
        Không thể tải danh sách khuyến mãi gói hội viên
      </div>
    );
      ...
      <div className="max-w-7xl mx-auto relative space-y-6">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
          <div className="flex flex-col gap-2">
            <h1 className="text-3xl font-bold tracking-tight">
              Khuyến mãi gói hội viên
            </h1>
            <p className="text-slate-600">
              Quản lý các chương trình khuyến mãi cho gói đăng ký hội viên
            </p>
          </div>

          <div className="flex flex-col sm:flex-row sm:items-center gap-3 w-full lg:w-auto">
            <Link href="/promotions-admin/plans/create">
        ...
        {isLoading ? (
          ...
          <AdminPromotionList
            ...
            onEdit={(promotion) => {
              router.push(`/promotions-admin/plans/${promotion.id}/edit`);
            }}
...
export default function PlanPromotionsPage() {
  return (
    <Suspense fallback={
