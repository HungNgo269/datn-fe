...
// Relative imports assuming we are in app/feature/plans/components
import { Plan, SubscriptionPlan, BillingInterval } from "../types/plans.types";
import { createPlan, updatePlan } from "../api/plans.api";
// Absolute import for schema as it is outside features
...
export function PlanForm({ planToEdit, onSuccess, onCancel }: PlanFormProps) {
  ...
  useEffect(() => {
    if (planToEdit) {
      ...
      const normalizedInterval =
        (planToEdit.interval?.toUpperCase() as BillingInterval) ||
        BillingInterval.MONTH;
      const normalizedPlan =
        (planToEdit.plan?.toUpperCase() as SubscriptionPlan) ||
  ...
  const mutation = useMutation({
    ...
    onError: (error: unknown) => {
      ...
      if (
        ...
        errorMessage =
          "Gói với loại và thời hạn này đã tồn tại. Vui lòng chọn thời hạn khác hoặc chỉnh sửa gói hiện có.";
      }
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="px-6 pb-6 space-y-5">
              ...
              <Select onValueChange={field.onChange} value={field.value}>
                <SelectTrigger
                  className={`h-11 rounded-xl border-slate-200 bg-slate-50/50 ${
                    errors.plan ? "border-rose-300 bg-rose-50/50" : ""
                  }`}
                >
      ...
      <div className="space-y-2">
        ...
        <Textarea
          ...
          className={`min-h-[80px] rounded-xl border-slate-200 bg-slate-50/50 transition-all focus:bg-white focus:ring-2 focus:ring-primary/20 focus:border-primary ${
            errors.description ? "border-rose-300 bg-rose-50/50" : ""
          }`}
        />
        {errors.description && (
          <p className="text-sm text-rose-500 flex items-center gap-1">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div className="space-y-2">
          ...
          <Input
            ...
            placeholder="99000"
            className={`h-11 rounded-xl border-slate-200 bg-slate-50/50 transition-all focus:bg-white focus:ring-2 focus:ring-primary/20 focus:border-primary ${
              errors.price ? "border-rose-300 bg-rose-50/50" : ""
            }`}
          />
          {errors.price && (
            <p className="text-sm text-rose-500 flex items-center gap-1">
      ...
      {/* Features Section */}
      <div className="space-y-2">
        <Label className="text-sm font-medium text-slate-700">Tính năng</Label>
        <div className="space-y-2">
          ...
          render={({ field }) => (
            <div className="flex items-center gap-3 h-11 rounded-xl border border-slate-200 bg-gradient-to-r from-slate-50 to-white px-4 transition-all hover:border-slate-300">
              ...
              >
                <span
                  className={
                    field.value ? "text-green-700 font-medium" : "text-slate-500"
                  }
                >
