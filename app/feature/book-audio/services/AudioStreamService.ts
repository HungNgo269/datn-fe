...
class AudioStreamService {
   ...
   */
  private async fetchAudioWithRetry(
    url: string, 
    attempt = 1,
    ...
    try {
      ...
      if (attempt < 3) {
        ...
        if (isRetryable) {
            ...
            return this.fetchAudioWithRetry(url, attempt + 1, onStateChange);
        }
      }
      throw error;
    } finally {
   ...
   * Uses manual fetch to handle retries and headers.
   */
  /**
   * Get audio source for playback.
   ...
   */
  public async fetchAndCache(chapterId: string) {
      try {
        const { url: audioUrl } = await getChapterAudio(chapterId);
        const { response } = await this.fetchAudioWithRetry(audioUrl, 1);
    ...
    if (chapterId) {
      try {
        ...
        if (!cached) {
           ...
           await this.fetchAndCache(chapterId);
        }
      } catch (error) {
